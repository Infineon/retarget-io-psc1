...
*
* \brief
* Provides APIs for retargeting stdio to UART hardware contained on the PSC1M3
* kits.
*
********************************************************************************
* \copyright
...
// The PSC1M3 UART driver is not necessarily thread-safe. To avoid concurrent
...
{
    while ((Cy_UART_CH_GetStatusFlag(cy_retarget_io_uart_obj.channel) &
            (CY_UART_CH_STATUS_FLAG_RECEIVE_INDICATION |
             CY_UART_CH_STATUS_FLAG_ALTERNATIVE_RECEIVE_INDICATION)) == 0U)
    {
        // Block indefinitely waiting for data in the receive buffer
    }

    // Read single character from the receive buffer
    *c = Cy_UART_CH_GetReceivedData(cy_retarget_io_uart_obj.channel);

    // Clear the receive buffer indication flag
    Cy_UART_CH_ClearStatusFlag(cy_retarget_io_uart_obj.channel,
                                CY_UART_CH_STATUS_FLAG_ALTERNATIVE_RECEIVE_INDICATION |
                                CY_UART_CH_STATUS_FLAG_RECEIVE_INDICATION);
    return CY_RSLT_SUCCESS;
}
...
static inline cy_rslt_t cy_retarget_io_putchar(char c)
{
    ...
    Cy_UART_CH_Transmit(cy_retarget_io_uart_obj.channel, c);
    return CY_RSLT_SUCCESS;
}
...
// cy_retarget_io_init
//--------------------------------------------------------------------------------------------------
cy_rslt_t cy_retarget_io_init(Cy_USIC_CH_t* base)
{
...
{
    return (Cy_UART_CH_GetStatusFlag(cy_retarget_io_uart_obj.channel) &
            CY_UART_CH_STATUS_FLAG_TRANSFER_STATUS_BUSY) > 0U;
}
